...
export function calculateMBTI(answers: Record<string, string>, language: string = 'en'): MBTIResult {
  const scores = {
    E: 0,
    I: 0,
    S: 0,
  ...
  const typeDetails = getMBTITypeDetails(type, language);
...
function getRussianTypeDetails(type: string): TypeDetails {
  const russianTypes: Record<string, TypeDetails> = {
    ...
    INFP: {
      ...
      strengths: ["Сильные ценности", "Творческое выражение", "Эмпатия"],
      growthAreas: [
        "Принятие решений",
        "Управление временем",
        "Самоутверждение",
    ...
    ESTJ: {
      ...
      growthAreas: ["Гибкость", "Эмоциональная осведомленность", "Терпение"],
    },
    ESFJ: {
      title: "Консул",
    ...
    ESFP: {
      ...
      strengths: ["Социальные навыки", "Позитивность", "Практическая помощь"],
      growthAreas: [
        "Долгосрочное планирование",
        "Принятие критики",
        "Фокус на задачах",
...
function getTypeDetails(type: string, lang: string): TypeDetails {
  const allTypeDetails = {
    ...
    // Inside mbti-calculator.ts, replace the entire 'ru' object with this:
    ru: {
      ENFP: {
...
function getPersonalityDetails(type: string, language: string = 'en') {
  const allTypeDetails = {
    ko: {
      ENFP: {
        title: "열정적인 활동가",
  ...
  // Select the map for the correct language, defaulting to English if the language is not found.
  const langMap = (allTypeDetails as any)[language] || allTypeDetails.en;
  // Select the details for the correct type, defaulting to INFP if the type is not found.
  const details = langMap[type] || langMap.INFP;
